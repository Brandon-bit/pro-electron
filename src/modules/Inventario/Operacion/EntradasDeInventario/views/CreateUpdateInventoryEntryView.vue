...
const {GetListaByClasificacionAction} = useInventoryEntriesActions()
...
const movements = ref<SelectOptionsType[]>([])
...
onMounted(async () => {
    ...
    if (!movements.value.length) {
        const data = await GetListaByClasificacionAction(1)
        movements.value = data.map(({ id, name }) => {
            return { id, label: name }
        })
    }
    ...
    if (inventoryEntryId) {
        await getInventoryEntryById(inventoryEntryId)
    }
})
...
watch(
    ...
    (newValues) => {
        if (!newValues || !newValues.stateId) return // evita valores vacíos
        resetForm({ values: { ...newValues } })
    },
    { immediate: false } // MUY IMPORTANTE
)
