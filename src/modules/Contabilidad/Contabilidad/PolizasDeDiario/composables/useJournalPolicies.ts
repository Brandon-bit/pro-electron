import useJournalPoliciesStore from '@/modules/Contabilidad/Contabilidad/PolizasDeDiario/store/journalPoliciesStore'
import { useModalStore } from '@/shared/stores/modal.store'
import type { ColumnTableType } from '@/shared/types/columnTableType'
import { h } from 'vue'
import { editTableButton, deleteTableButton } from '@/utils/tableButtons'

const useJournalPolicies = () => {
    const journalPoliciesStore = useJournalPoliciesStore()
    const modalStore = useModalStore()

    const getTableColumns = (): ColumnTableType[] => {
        const columns = [
            {
                header: 'Folio',
                accessorKey: 'folio',
                cell: ({ row }: any) => {
                    return h('span', { class: 'font-medium' }, row.original.folio)
                }
            },
            {
                header: 'Fecha',
                accessorKey: 'date',
                cell: ({ row }: any) => {
                    const date = new Date(row.original.date)
                    return h('p', {}, date.toLocaleDateString('es-MX'))
                }
            },
            {
                header: 'Concepto',
                accessorKey: 'concept',
                cell: ({ row }: any) => {
                    return h('span', {}, row.original.concept)
                }
            },
            {
                header: 'Total',
                accessorKey: 'total',
                cell: ({ row }: any) => {
                    const total = row.original.total
                    return h(
                        'span',
                        { class: 'font-mono' },
                        `$${total.toLocaleString('es-MX', { minimumFractionDigits: 2 })}`
                    )
                }
            },
            {
                header: 'Usuario',
                accessorKey: 'user',
                cell: ({ row }: any) => {
                    return h(
                        'span',
                        { class: 'badge badge-secondary' },
                        row.original.user
                    )
                }
            },
            {
                header: 'Acciones',
                accessorKey: '',
                cell: ({ row }: any) => {
                    const data = row.original
                    const editModal = () => {
                        journalPoliciesStore.setData(data)
                        modalStore.open(journalPoliciesStore.modalId, {
                            type: 'EDIT',
                            title: 'Editar póliza de diario'
                        })
                    }
                    const deleteModal = () => {
                        journalPoliciesStore.setData(data)
                        modalStore.open(journalPoliciesStore.modalId, {
                            type: 'DELETE',
                            title: 'Eliminar póliza de diario'
                        })
                    }
                    const editButton = editTableButton(editModal)
                    const deleteButton = deleteTableButton(deleteModal)
                    return h('div', { class: 'flex gap-4 justify-center' }, [
                        editButton,
                        deleteButton
                    ])
                }
            }
        ]

        return columns
    }

    return { getTableColumns }
}

export default useJournalPolicies
