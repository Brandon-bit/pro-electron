...
import type { MaskSegment } from '@contabilidad/Configuracion/MaskConfig/types/maskConfigTypes'

const { getMaskConfig, updateMaskConfig, createMaskConfig } = useMaskConfigActions()
const maskConfigStore = useMaskConfigStore()

// Estado del formulario
const segments = ref<MaskSegment[]>([])
const separator = ref('-')
const maxDepth = ref(6)
const allowFlexibleLength = ref(false)
...
const loadConfig = async () => {
    ...
    try {
        ...
        if (config) {
            ...
            allowFlexibleLength.value = config.allowFlexibleLength
            configExists.value = true
            
            // Actualizar store
            maskConfigStore.setConfig(config)
        } else {
            // No hay configuración aún - esto es normal en la primera vez
...
const saveConfig = async () => {
    ...
    try {
        if (configExists.value && configId.value) {
            ...
            await updateMaskConfig(configId.value, {
            ...
            showNotification('Configuración de máscara actualizada exitosamente', 'success')
        } else {
            // Crear nueva configuración
            const created = await createMaskConfig({
                format: formatPreview.value,
                separator: separator.value,
                maxDepth: maxDepth.value,
                allowFlexibleLength: allowFlexibleLength.value,
            ...
            configExists.value = true
            showNotification('Configuración de máscara creada exitosamente', 'success')
        }
        
        // Recargar configuración para actualizar store
        await loadConfig()
    } catch (error) {
...
<template>
    <div class="space-y-6">
        ...
        <div v-else class="space-y-6">
            <!-- Mensaje cuando no existe configuración -->
            <div v-if="!configExists" class="alert alert-warning">
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" class="stroke-current shrink-0 w-6 h-6">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"></path>
                </svg>
                <div>
                    <h3 class="font-bold">Configuración Inicial de Máscara</h3>
                    <div class="text-sm">
                        Esta es la primera vez que configuras la máscara contable. La estructura que establezcas aquí determinará el formato de todos los códigos de cuenta del catálogo contable.
                    </div>
                </div>
            </div>
