...
export const employeeSchema = z.object({
    ...
    accountNumber: optionalStringValidator(
        'El número de cuenta debe tener al menos 10 dígitos',
        10
    ),
    clabeNumber: optionalStringValidator(
        'La CLABE debe tener exactamente 18 dígitos',
        18
    ).refine((val) => !val || val.length === 0 || val.length === 18, {
        message: 'La CLABE debe tener exactamente 18 dígitos'
    }),
    
    // Documentos (opcionales)
    employeeDocuments: z.any().optional(),
    psychometricTests: z.any().optional()
})
