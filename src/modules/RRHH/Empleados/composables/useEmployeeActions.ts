import { mapEmployeeRequest } from '@/modules/RRHH/Empleados/composables/mappingEmployees'
import {
    EmployeeRequest,
    EmployeeDTO,
    SelectOptionDTO
...
// Mock banks data
const mockBanks: SelectOptionDTO[] = [
    { id: 1, label: 'BBVA México' },
    { id: 2, label: 'Santander' },
    { id: 3, label: 'Banorte' },
    { id: 4, label: 'HSBC' },
    { id: 5, label: 'Citibanamex' },
    { id: 6, label: 'Scotiabank' },
    { id: 7, label: 'Inbursa' }
]
...
export const useEmployeeActions = () => {
    ...
    const getEmployeeById = async (id: number): Promise<EmployeeRequest> => {
        ...
        return {
            ...
            department:
                (mockDepartments.find((d) => d.label === employee.department)?.id as number) || 2,
            position: employee.position,
            salary: employee.salary,
            contractType:
                (Object.keys(contractTypeMap).find(
                    (key) => contractTypeMap[Number(key)] === employee.contractType
                ) as unknown as number) || 1,
            reportsTo: employee.reportsTo
                ? (mockSupervisors.find((s) => s.label.includes(employee.reportsTo as string))
                      ?.id as number)
                : undefined
        }
    }
    ...
    const getDepartments = async (): Promise<SelectOptionDTO[]> => {
        ...
        const { useDepartmentActions } = await import(
            '@/modules/RRHH/Departamentos/composables/useDepartmentActions'
        )
    ...
    const getBanksForSelect = async (): Promise<SelectOptionDTO[]> => {
        console.log('Fetching banks for select')

        // TODO: Implement axios call
        // const response = await axiosInstance.get('/banks')
        // return response.data.map(bank => ({ id: bank.id, label: bank.nombre }))
