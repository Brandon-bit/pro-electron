...
class AuthService {
  ...
  async login(email: string, password: string): Promise<{ success: boolean; token?: string; message?: string }> {
    try {
      ...
      const response = await axiosApiInstance.post<LoginResponse>('/auth/login', loginData);
  ...
  async validateToken(): Promise<{ valid: boolean; user?: User; message?: string }> {
    try {
      ...
      const response = await axiosApiInstance.get<ValidateTokenResponse>('/auth/validate');
  ...
  async refreshToken(): Promise<{ success: boolean; token?: string; message?: string }> {
    try {
      ...
      const response = await axiosApiInstance.post<RefreshTokenResponse>('/auth/refresh');
